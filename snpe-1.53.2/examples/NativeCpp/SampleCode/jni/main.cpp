...
int main(int argc, char** argv)
{
    ...
    int opt = 0;
    while ((opt = getopt(argc, argv, "hi:d:o:b:s:z:r:l:u:c")) != -1)
    {
        switch (opt)
        {
            case 'h':
    ...
    if(useUserSuppliedBuffers)
    {
       ...
       if( bufferType == USERBUFFER_TF8 || bufferType == USERBUFFER_TF16 )
       {
          ...
          createInputBufferMap(inputMap, applicationInputBuffers, snpeUserBackedInputBuffers, snpe, true, bitWidth);
          ...
          for( size_t i = 0; i < inputs.size(); i++ )
          {
             ...
             if(!loadInputUserBufferTfN(applicationInputBuffers, snpe, inputs[i], inputMap, bitWidth))
             {
                 return EXIT_FAILURE;
             }
       ...
       else if( bufferType == USERBUFFER_FLOAT )
       {
          ...
          {
             std::unordered_map <std::string, std::vector<uint8_t>> applicationInputBuffers;
             createInputBufferMap(inputMap, applicationInputBuffers, snpeUserBackedInputBuffers, snpe, false, bitWidth);

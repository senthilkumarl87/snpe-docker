...
ir_consts_to_dlc = {
    ...
    op_adapter.IRPaddingStrategies.PADDING_SIZE_IMPLICIT_SAME_BEGIN: modeltools.PADDING_SIZE_IMPLICIT_SAME,
...
class DlcDetectionOutputTranslation(BackendTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        caffe_converter = True if graph.src_axis_order == AxisOrders.CAFFE else False
        if caffe_converter is True:
            backend.model.add_ssd_detection_output_layer(node.op.name,
                                                         node.input_names,
                                                         node.output_names,
                                                         node.op.output_dims,
                                                         node.op.num_classes,
...
class DlcResizeTranslation(BackendTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        if node.op.half_pixel_centers is True:
            raise ValueError("Conversion Error: SNPE does not support half_pixel_centers param for Resize Layer.")
...
class DlcSoftmaxTranslation(BackendTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        input_rank = graph.get_buffer(node.input_names[0]).rank()
        if node.op.axis not in [input_rank - 1, -1]:

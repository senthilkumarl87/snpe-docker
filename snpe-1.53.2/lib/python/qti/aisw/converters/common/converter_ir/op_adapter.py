...
class Op(ABC):
    ...
    @abstractmethod
    def infer_shape(self, input_shapes: list, num_outputs: int, axis_order) -> list:
        raise NotImplementedError(
...
class InputOp(Op):
    ...
    def __init__(self, name, shape, **kargs):
        ...
        self.addattr('input_dtype', kargs, float)
...
class BatchToSpaceOp(Op):
    ...
    def infer_shape(self, input_shapes: List[List[int]], num_outputs: int, axis_order) -> List[int]:
...
class ConvolutionOp(Op):
    ...
    def __init__(self, name, weights, bias, bias_op_name=None, **kargs):
        Op.__init__(self, name, self.TRANSLATION_KEY)
        self.weights = weights
        self.bias = bias
        ...
        self.addattr('padding_size_strategy', kargs,
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        batch_size, input_height, input_width, input_depth = axis_order.extract_spatial_dims(
            input_shapes[0])
        filter_size_height, filter_size_width, _, _ = axis_order.extract_conv_weights_dims(
            self.weights.shape)
...
class DeconvolutionOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        ...
        filter_size_height, filter_size_width, _, _ = axis_order.extract_deconv_weights_dims(
            self.weights.shape)
...
class ElementwiseOp(Op):
    ...
    def infer_shape(self, input_shapes: list, num_outputs: int, axis_order) -> list:
        return [translation_utils.get_broadcasted_shape(input_shapes, axis_order)]
...
class LstmOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        ...
        if len(input_shape) == 3:
            batch_size, time_steps, _ = axis_order.extract_time_series_dims(input_shape)
        output_depth = self.hidden_size  # Num of hidden units
        output_dims.append(
            axis_order.format_time_series_output_shape(batch_size=batch_size,
                                                       time_steps=time_steps,
                                                       feature=output_depth)
...
class MatMulOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        ...
        output_shape = np.matmul(np.random.rand(*input1), np.random.rand(*input2)).shape
        return [output_shape]
...
class OneHotOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        ...
        output_shape = [list(input_shapes[0][:self.axis]) + [self.depth] + list(input_shapes[0][self.axis:])]
        return output_shape
...
class PixelShuffleOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        ...
        output_shape = [batch,
                        int(height * self.upscale_factor),
                        int(width * self.upscale_factor),
                        int(depth / (self.upscale_factor ** 2))]
        return [output_shape]
...
class PreluOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        ...
        output_shape = translation_utils.get_broadcasted_shape(prelu_shapes, axis_order)
        return [output_shape]
...
class RNormOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        if not self.across_channels:
            ...
            if input_rank != 4:
                raise ValueError(
                    "Input rank (%d) must equal 4 for WITHIN LRN on node %s"(input_rank, self.name))
...
class StridedSliceOp(Op):
    ...
    def infer_shape(self, input_shapes, num_outputs, axis_order):
        ...
        for i in range(len(input_shapes[0])):
            if not self.shrink_axis_mask & (1 << i):
                output_shape.append(
                    int(math.ceil(float(self.end[i] - self.begin[i]) / self.strides[i])))
        return [output_shape]

...
class Buffer(object):
    ...
    def populate_axis_format(self, axis_order, encodings):
        ...
        if (InputEncodings.OTHER in encodings and (len(self.get_buf_dims()) == 4 or len(self.get_buf_dims()) == 3)) \
                or (InputEncodings.TIME_SERIES not in encodings and len(self.get_buf_dims()) == 3):
...
class IROpGraph(object):
    ...
    def get_input_encoding(self, input_name):
        ...
        return self.inputs_encoding_dict.get(input_name_, InputEncodings.BGR)
    ...
    def merge_quantization_params(self, source_op_name, destination_op_name, pre_merge_dest_tensor_name,
        ...
        if source_op_name in self.quantization_params:
            ...
            log_assert(len(self.quantization_params[source_op_name][encoding_type]) == 1,
    ...
    def replace(self, old_op, new_op):
        ...
        input_shapes = []
        # TODO Replace set(input_buffers) with something that enforces order
        for buf in set(input_buffers):
    ...
    def add_input(self, name, shape, axis_format=None, input_type=None, input_dtype=float):
        if not input_type:
            input_type = self.get_input_type(name)

        input_encoding_in = self.get_input_encoding(name)
    ...
    def get_output_nodes_of_graph(self):
        output_nodes = []
        passed_output_node_names = self.output_nodes[:]

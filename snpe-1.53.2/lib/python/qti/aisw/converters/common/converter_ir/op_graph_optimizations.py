...
REMOVE_IDENTITY_NOOP = "REMOVE_IDENTITY_NOOP"
...
supported_opt_list = [SQUASH_SCALE, SQUASH_PROD, SQUASH_DIV, SQUASH_SUM, SQUASH_SUB, SQUASH_BATCHNORM, FOLD_CONCATS,
                      MATCH_CHANNELSHUFFLE, MATCH_GELU, MATCH_HARDSWISH, AXES_TO_SPATIAL_FIRST_ORDER,
                      REMOVE_IDENTITY_NOOP, REMOVE_NOOP, REMOVE_CAST_NOOP, ADD_QPARAMS, CHAIN_ELTWISE_OPS,
                      ADJUST_NMS_FEATURE_DIMS, EXTRACT_COLOR_TRANSFROM,OPTIMIZE_NEG, MATCH_SPACETODEPTH,
                      PREPROCESS_ROI_POOL_INPUTS, PREPROCESS_LSTM_OPS, SQUASH_PAD, MERGE_LOW_LEVEL_OPS_TO_LAYERS,
                      INJECT_CAST_FOR_GATHER, REMOVE_QUANT_NODES, SQUASH_QUANT_NODES, ALIGN_MATMUL_RANKS]
...
class IROptimizations(object):
    class ArgParser(ArgParserWrapper):
        def __init__(self, **kwargs):
            ...
            self.add_optional_argument("--inject_cast_for_gather",
                                       ...
                                       help=argparse.SUPPRESS,
                                       action="store_true")
            self.add_optional_argument("--keep_identity_output",
                                       default=False,
                                       help=argparse.SUPPRESS,
                                       action="store_true")
            ...
            q_group.add_argument('--keep_quant_nodes', default=False, action="store_true",
                                 help='Use this option to keep activation quantization nodes in the graph rather than stripping them.')
    ...
    def __init__(self, args):
        ...
        self.keep_identity_output = args.keep_identity_output
        self.use_convert_quantization_nodes = args.use_convert_quantization_nodes
        self.keep_quant_nodes = args.keep_quant_nodes
    ...
    def optimize(self, graph):
        ...
        # Attempts to squash identity noops that are graph outputs if keep_identity_output is set
        # Otherwise the identity output noops are pruned.
        OptimizationTranslations.apply_method_to_all_ops(REMOVE_IDENTITY_NOOP, graph,
                                                         keep_identity_output=self.keep_identity_output,
                                                         fail_if_no_method=False)
        ...
        # Apply preprocessing to LSTM inputs
    ...
    for node_tuple in matched_node_list:
        ...
        if nn_buf.axis_format == AxisTracker.AxisFormat.NCS:
            if nn_op.hasattr("weights") and len(nn_op.weights.shape) == 4:
                ...
                if nn_op.type == op_adapter.ConvolutionOp.TRANSLATION_KEY:
                    nn_op.weights = numpy.transpose(nn_op.weights, (2, 3, 1, 0))  # to HWIO
                elif nn_op.type == op_adapter.DeconvolutionOp.TRANSLATION_KEY:
                    nn_op.weights = numpy.transpose(nn_op.weights, (2, 3, 0, 1))  # to HWIO
            if const_op is not None and len(const_op.tensor.shape) == 4:
                const_op.tensor = numpy.transpose(const_op.tensor, (0, 2, 3, 1))  # to NHWC

        # separate conditionals according to which arithmetic operation needs to happen
        if node_type in OPS_HAVING_BIAS:
            scale_bias = const_op.tensor
            nn_op.bias = numpy.atleast_1d((nn_op.bias + scale_bias).squeeze())
        elif node_type in OPS_HAVING_WEIGHTS:
        ...
        if nn_buf.axis_format == AxisTracker.AxisFormat.NCS:
            if nn_op.hasattr("weights") and len(nn_op.weights.shape) == 4:
                if nn_op.type == op_adapter.ConvolutionOp.TRANSLATION_KEY:
                    nn_op.weights = numpy.transpose(nn_op.weights, (3, 2, 0, 1))  # to OIHW
                elif nn_op.type == op_adapter.DeconvolutionOp.TRANSLATION_KEY:
                    nn_op.weights = numpy.transpose(nn_op.weights, (2, 3, 0, 1))  # to IOHW
            if const_op is not None and len(const_op.tensor.shape) == 4:
                const_op.tensor = numpy.transpose(const_op.tensor, (0, 3, 1, 2))  # to NCHW
...
def validate_eltwise_pattern(graph, nodes_tuple, mode):
    ...
    for name_ in node.input_names:
        ...
        if op_.type == op_adapter.ConstantOp.TRANSLATION_KEY:
            const_op = op_
...
class OptimizeBatchnormTranslation(OptimizationTranslationBase):
    ...
    def squash_batchnorm(graph):
        ...
        for node_tuple in matched_node_list:
            ...
            conv_node.op.bias = numpy.atleast_1d((conv_node.op.bias * bn_node.op.weights + bn_node.op.bias).squeeze())
...
class OptimizeGatherTranslation(OptimizationTranslationBase):
    ...
    def inject_cast_for_gather(self, node, graph):
        ...
        cast_op = op_adapter.CastOp(name=cast_node_name, to_type=numpy.dtype("int32"))
        ...
        if graph.has_buffer(cast_node_name):
            ...
            old_consumers = list(cast_buffer.consumers)
            old_consumers.append(node)
...
class OptimizeLstmTranslation(OptimizationTranslationBase):
    ...
    def preprocess_lstm_ops(graph):
        ...
        for nodes_tuple in matched_node_list:
            ...
            if len(input_shape) == 3:
                ...
                if candidate_reshape_node.op.type == op_adapter.ReshapeOp.TRANSLATION_KEY:
                    ...
                    if len(output_buffers[0].consumers) == 1 and input_buffers[0].shape == [batch_size, input_size]:
                        input_reshape_needed  = not graph.squash(candidate_reshape_node, input_buffers[0].name)

            else:
                 raise ValueError('Unsupported input rank for LSTM node {}, expected 3, got {}.'.format(
                    lstm_node_name, len(input_shape)))

            # Requires intial_h and initial_c inputs to be present
            if len(lstm_node.input_names) != 3:
                raise ValueError('Unsupported number of inputs for LSTM node {}, expected 3, got {}.'.format(
            ...
            initial_state_reshape_needed = len(initial_h_shape) != 2
            ...
            output_size = lstm_node.op.hidden_size
...
class OptimizeMatmulTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        for input_name in node.input_names:
            ...
            if input_buf.producer.op.type != op_adapter.PermuteOp.TRANSLATION_KEY:
                if input_buf.axis_format == AxisTracker.AxisFormat.NCS:
                    pass
    ...
    def align_matmul_input_ranks(node, graph):
        ...
        if inputs[0].rank() != inputs[1].rank():
            ...
            log_debug1("This matmul impl requires identical rank rank, reshaping {} to {}".format(lower_rank_input_buf.shape, new_shape))
...
class OptimizeNoopTranslation(OptimizationTranslationBase):
    def __init__(self):
        ...
        self.register_method(REMOVE_IDENTITY_NOOP, self.remove_identity_noop)
    ...
    @staticmethod
    def remove_identity_noop(node, graph, keep_identity_output=False):
        def is_node_squashable():
            input_buffer = graph.get_buffer(node.input_names[0])
            if len(input_buffer.consumers) == 1:
                return True
            else:
                log_debug2("Cannot squash node {}, as exactly one consumer is required to squash into previous. Got {}".format(
                        node.op.name, len(input_buffer.consumers)))
                return False

        if (graph.is_output_node(node) and not is_node_squashable()) or (not keep_identity_output and graph.is_output_node(node)):
            log_warning("Pruning identity node {}".format(node.op.name))
            graph.prune(node)
        else:
            graph.squash(node, input_name=node.input_names[0])
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if not translation_utils.broadcastable(input_buf.shape, coeff_shape):
            ...
            if input_permute_order is not None:
                ...
                node.op.coeff = numpy.ascontiguousarray(numpy.transpose(node.op.coeff, coeff_permute_order))
                coeff_shape = node.op.coeff.shape
...
class OptimizeSoftmaxTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if input_buf.axis_format in format_to_permute_order:
            axis_map = format_to_permute_order[input_buf.axis_format]
            node.op.axis = axis_map[node.op.axis]
            print('mapping axis from {} to {}: '.format(node.op.axis,  axis_map[node.op.axis]))
...
class OptimizeFakeNonMaxSuppressionTranslation(OptimizationTranslationBase):
    ...
    def merge_low_level_ops_to_layers(self, graph):
        ...
        for node_tuple in matched_node_list:
            ...
            gather_node = node_tuple[-2]
            reshape_output_buf = graph.get_output_buffers(reshape_node)[0]
            pruned_nodes = []
            consumer_succors = []
            for consumer in reshape_output_buf.consumers:
                if consumer.op.type == 'gather':
                    consumer_input_names = consumer.input_names
                    gather_data_inputs = [input_name for input_name in consumer_input_names if input_name != reshape_output_buf.name]
                    ...
                    consumer_succors.append(graph.get_output_buffers(consumer)[0].consumers)
            for node in pruned_nodes:
                graph.prune(node, force_remove=True)
            graph.prune(reshape_node, force_remove=True)
            graph.prune(gather_node, force_remove=True)
            graph.prune(fake_nms_node, force_remove=True)
            idx_to_insert = 0
            ...
            for idx, succs in enumerate(consumer_succors):

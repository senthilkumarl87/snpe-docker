...
class OnnxPowTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        if is_broadcast(src_op, graph):
            log_warning(code_to_message.get_warning_message("WARNING_BROADCAST_POW"))

        power_tensor_name = src_op.input[1]
        if graph.weights.has(power_tensor_name):
            power_tensor = graph.weights.fetch(power_tensor_name).astype(numpy.float32)
        else:
            raise ValueError(
                "Power tensor on {} node {} was not extracted".format(src_op.op_type, src_op.name))

        return op_adapter.PowerOp(str(src_op.name), power=power_tensor)

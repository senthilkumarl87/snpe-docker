...
class OnnxConvTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, graph):
        ...
        log_debug(code_to_message.get_debugging_message('DEBUG_WEIGHT_DIMS')(weights.shape))
        ...
        return op_adapter.ConvolutionOp(src_op.name,
                                        weights,
                                        bias,
                                        bias_op_name=input_names[2] if len(
                                            input_names) > 2 else None,
                                        pady_before=params.pads[0],
                                        pady_after=params.pads[num_input_pads],
                                        padx_before=params.pads[1],
                                        padx_after=params.pads[1 + num_input_pads],
    ...
    def extract_parameters(self, src_op, graph):
        ...
        if len(input_names) > 2:
            ...
            bias = numpy.zeros(weights.shape[1],
                               dtype=numpy.float32)  # take the second dim because Onnx weights for
        ...
        if len(input_names) > 2:
            ...
            # Take the second dimension and multiply by groups since Onnx is NCHW
            # and weights.shape[1] is M/groups
            bias = numpy.zeros(weights.shape[1] * params.group,
                               dtype=numpy.float32)
        if params.kernel_shape:
            log_assert(tuple(params.kernel_shape) == weights.shape[2:],
                       code_to_message.get_error_message("ERROR_KERNEL_SHAPE_DIFFERS_FROM_WEIGHTS"))
        ...
        params.pads = op_adapter.DeconvolutionOp.calc_deconv_padding_size(weights.shape[2:],
                                                                          params.dilations,
                                                                          params.strides,
                                                                          padding_mode,
                                                                          params.pads,

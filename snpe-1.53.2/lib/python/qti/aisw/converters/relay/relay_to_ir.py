...
class RelayConverterContext:
    ...
    def get_output_names(self, expr: relay.expr, num_outputs: int=None):
        ...
        expr_text = "{}".format(expr)
        re_comment = re.compile(r'''(/\*.*?\*/)''', re.VERBOSE)
        key = hash(re_comment.sub('', expr_text).replace(' ', ''))
    ...
    def add_op_to_graph(self,
        ...
        :return: QuIR OpNode
        """
        # This is a special case seen for Pack(ExpandDims)Op
        # Input buffer is added twice and hence the condition check
        # to skip if the buffer is already present
        if not QUIR_GRAPH.has_buffer(output_names[0]):
            key = hash("{}".format(expr).replace(' ', ''))
            ...
            log_warning("Duplicate Buffer name {} found. Skipping the addition of op to IROpGraph".format(
                        output_names[0]))
...
class RelayConverterFrontend(ConverterFrontend):
    ...
    def dump_relay_data(self, args):
        ...
        full_mod_path = os.path.join(path, "mod.txt")
        self.dump_mod(full_mod_path)
        ...
        def preprocess_expr(expr, output_name, tuples_dict):
            if isinstance(expr, relay.TupleGetItem) or \
                ...
                if isinstance(new_expr, relay.expr.Call):
                    k = hash("{}".format(new_expr).replace(' ', ''))
                    log_verbose("k:\n{}\n", new_expr)
    ...
    def add_constant(expr: relay.expr):
        ...
        if key not in EXPR_TO_OP_OUT_NAMES:
            ...
            constant_array = expr.data.asnumpy()

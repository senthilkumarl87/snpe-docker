...
class CastLayerResolver(LayerResolver, object):
    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        for match in matches:
            cast_op = match['root']
            from_type = np.dtype(cast_op.get_attr(self.TF_ATTRIBUTE_SRC_TENSOR_TYPE).as_numpy_dtype)
            to_type = np.dtype(cast_op.get_attr(self.TF_ATTRIBUTE_DST_TENSOR_TYPE).as_numpy_dtype)

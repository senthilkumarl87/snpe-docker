...
from qti.aisw.converters.common.converter_ir.op_adapter import ConvolutionOp, ResizeOp, PadOp, IRPaddingStrategies
...
class ConvolutionLayerResolver(LayerResolver, object):
    ...
    class Descriptor(LayerDescriptor):
        ...
        def is_input_tensor(self, op, tensor):
            ...
            # if not grouped conv case
            if len(self.input_ops) == 1:
                input_op = self.input_ops[0]
                # handle cases when fakequant and identity are ignored by determining that the fakequant or identity
                # op's input tensors were off the first input path
                if op != input_op and input_op.inputs[0].op != op:
                    if not len(input_op.inputs[0].op.inputs) or not len(op.inputs):
                        return False
                    expected_act_input = GraphHelper.get_stripped_input(input_op.inputs[0].op.inputs[0],
    ...
    def get_biases(self, graph_helper, conv_op, bias_op):
        ...
        if biases_tensor.op.type not in ['Identity', 'Const'] and \
                not graph_helper.check_tensor_const_origin(biases_tensor)[0]:
    ...
    def get_weights(self, graph_helper, conv_op):
        ...
        if weights_tensor.op.type not in ['Identity', 'Const', 'Split', 'FakeQuantWithMinMaxVars'] and \
                not graph_helper.check_tensor_const_origin(weights_tensor)[0]:
    ...
    def resolve_layer(self, graph_matcher, graph_helper):
        ...
        for match in matches:
            ...
            d = DilatedDepthwiseConvolutionLayerResolver.Descriptor(
                    str(conv_op.name), consumed_nodes, conv_op, bias_op, output_op, strides, padding, weights,
                    biases, explicit_pads=pads, output_names=output_names)
...
class ConvolutionLayerBuilder(LayerBuilder):
    def build_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        log_debug(code_to_message.get_debugging_message('DEBUG_WEIGHT_DIMS')(descriptor.weights.shape))

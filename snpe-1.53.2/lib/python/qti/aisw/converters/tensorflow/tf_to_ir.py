...
class TFConverterFrontend(ConverterFrontend):
    class ArgParser(ConverterFrontend.ArgParser):
        def __init__(self, **kwargs):
            ...
            self.add_required_argument('--out_node', type=str, action='append',
            ...
            self.add_optional_argument("--match_lstms", action='store_true', default=True,
    ...
    def __init__(self, session, args, *, custom_op_factory=None):
        ...
        if not args.match_lstms:
            layers.layer_resolvers.remove(layers.LstmLayerResolver)
    ...
    def _convert_input_layers(self):
        ...
        for model_input in self._context.inputs:
            ...
            input_nodes.append(self.graph.add_input(layer_name, shape, input_type=layer_type))

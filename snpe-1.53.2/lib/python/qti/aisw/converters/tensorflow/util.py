...
class GraphHelper(object):
    ...
    def get_stripped_input(input_tensor, inputs_types_to_ignore):
        ...
        """
        log_assert(len(inputs_types_to_ignore), "Requested to traceback Op input path by ignoring specified "
    ...
    def check_op_const_origin(self, op):
        ...
        while queue:
            ...
            for input_op in head.inputs:
                if input_op.op.type == 'Placeholder' or input_op.op in self.get_graph_input_ops():
...
def get_const_op_value(op):
    if op.type != 'Const':
        raise TypeError("Expected Const type to resolve tensor value. Got {} for op {}".format(op.type, op.name))
    ...
    if len(tensor.tensor_content):
        ...
        tensor_val = np.frombuffer(tensor.tensor_content, dtype=tensor_type.as_numpy_dtype)

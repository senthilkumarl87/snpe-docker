...
def convert_to_dlc(pb_filename, model_dir, tensorflow_dir, runtime, udo, enable_htp):
    ...
    if ('dsp' == runtime or 'aip' == runtime or 'all' == runtime):
        ...
        if udo:
            x86_lib_path = os.path.join(model_dir, UDO.UDO_PACKAGE, 'libs', 'x86-64_linux_clang')
            if enable_htp:
                x86_lib_dsp_path = os.path.join(model_dir, "udo_dsp", UDO.UDO_PACKAGE, 'libs', 'x86-64_linux_clang')
                if 'LD_LIBRARY_PATH' in os.environ:
                    os.environ['LD_LIBRARY_PATH'] = os.path.abspath(os.environ['LD_LIBRARY_PATH']) + ":" + x86_lib_path \
                                                    + ":" + x86_lib_dsp_path
                else:
                    os.environ['LD_LIBRARY_PATH'] = x86_lib_path + ":" + x86_lib_dsp_path
            else:
                if 'LD_LIBRARY_PATH' in os.environ:
                    os.environ['LD_LIBRARY_PATH'] = os.path.abspath(os.environ['LD_LIBRARY_PATH']) + ":" + x86_lib_path
                else:
                    os.environ['LD_LIBRARY_PATH'] = x86_lib_path
...
def setup_assets(inception_v3_data_dir, download, runtime, udo, enable_htp):
    ...
    if(runtime == 'all') and not udo:
        raise RuntimeError('--udo flag needed to use \'all\' runtime selection')
    if download:
    ...
    if udo:
        is_quantized = False
        UDO.setup_udo(model_dir, runtime, is_quantized)
        if runtime == 'dsp' or runtime == 'aip' or runtime == 'all':
            path_dsp = os.path.join(model_dir, "udo_dsp")
            if not os.path.isdir(path_dsp):
                os.mkdir(path_dsp)
            UDO.setup_udo(path_dsp, runtime, not is_quantized, enable_htp)


    convert_to_dlc(pb_filename, model_dir, tensorflow_dir, runtime, udo, enable_htp)
...
if __name__ == '__main__':
    ...
    try:
        setup_assets(args.assets_dir, args.download, args.runtime, args.udo, args.htp)
    except Exception as err:
        sys.stderr.write('ERROR: %s\n' % str(err))

...
def setup_udo(udo_package_path, runtime, is_quantized, enable_htp = False):
    ...
    compile_udo_package(udo_package_path, runtime, is_quantized, enable_htp)
...
def set_udo_impl(udo_package_path, is_quantized, enable_htp):
    ...
    if enable_htp:
        ...
        package_libs = [os.path.join('DSP_V68', 'SoftmaxImplLibDsp.cpp')]
...
def compile_udo_package(udo_package_path, runtime, is_quantized, enable_htp):
    if not is_quantized:
        if runtime == 'cpu':
